...
- Missing `grappelli`: install `django-grappelli==3.0.8` (or remove the URL include).
- 401s on API calls: ensure JWT tokens are stored as `access`/`refresh` in `localStorage` (handled by frontend auth context).
- WebSocket closes immediately: verify query parameter uses a valid **access** token, not refresh, and that both user IDs are embedded in the `room_name` (`<smallerId>_<greaterId>`).

---

**Next steps**: extend seed data via Django fixtures, add automated tests (currently sparse in `events/tests.py` and `jobs/tests.py`), and document environment-specific settings for production.
